# CargoSense v3: An Intelligent Freight Forecasting & Vessel Chartering Optimization Platform for Bulk Raw Material Imports to India's East Coast
...
CargoSense is a three-layer decision-support platform. Layer 3 now has **four** modules (idle/deadheading advisor added in v3):

```
┌───────────────────────────────────────────────────────────────────────┐
│  LAYER 3 — Decision & Optimization Engine                             │
│  Chartering-timing recommender + MILP procurement optimizer +         │
│  cross-PSU pooling module + Idle/Deadheading Advisor (NEW)            │
├───────────────────────────────────────────────────────────────────────┤
...
│  historical CP fixtures — modeled port-and-route-agnostic (Sec 1.1)   │
└───────────────────────────────────────────────────────────────────────┘
```
...
Backend API:      Python (FastAPI) — forecasts, optimizer results,
                 /retrain, /generate-tender, /idle-advisor endpoints
...
|---|---|---|
| **Phase 1 (Hackathon prototype)** | Prophet+XGBoost forecast on Australia–Paradip; 3-constraint+quality-term MILP; CSV-driven pooling; Idle/Deadheading Advisor on the same engine; real generated Tender Specification document; port draft constraints refreshed against current published figures | Hackathon timeline |
| **Phase 2 (Pilot)** | MoU-based integration with one PSU's historical CP fixture data; real (not public-proxy) backtest; extend port-constraint fidelity from Paradip to all 7 named ports (Section 1.1); extend origin coverage to Indonesia/US/Mozambique/Russia; TFT sequence model; interactive scenario simulator; real MSTC/e-tender API hook; live Baltic Exchange FFA feed | 3–6 months |
| **Phase 3 (Scale)** | Multi-plant rollout, live Baltic Exchange + AIS subscription, PCS1x live berth integration, Kafka/Airflow/Kubernetes stack, CVC-compliant audit logging | 6–12 months |
...
**Financial:**
- **Backtested, not asserted**: the headline figure comes from running the fix-now/wait/hedge logic retroactively against 2 years of actual public BDI/Panamax index history (Section 10c) versus a naive "charter immediately every time" baseline — demonstrated on real historical market data, the standard a shipping-literate judge will hold any claim to.
- **Illustrative scale**: even at current coking coal import volumes (tens of millions of tonnes annually, projected to ~160 MT/year by 2030), a 3–5% improvement in freight-cost timing at typical $18–30/tonne rates implies savings in the hundreds of crores annually across SAIL, RINL, and (if extended) private steel majors — explicitly secondary to, and illustrative of, the backtested figure above.
...
1. Pull 2 years of actual public weekly Baltic Panamax/Capesize sub-index values, bunker (VLSFO) prices, and IMD's published cyclone-season records.
2. Re-run the fix-now/wait/hedge logic retroactively, week by week, against a synthetic but realistic recurring demand schedule.
3. Compare cumulative modeled landed cost against (a) always-charter-immediately and (b) last-value-carried-forward.
4. Report **modeled ₹-per-tonne saved, cumulative over the backtest window**, checked against known shock events (2021 supercycle, 2020 COVID collapse, 2023 Red Sea/Panama disruptions) to show whether the P90 band flagged risk *ahead of* each event.

**Why lead with this:** "Backtested against 2 years of real Baltic index history, this logic would have saved ~₹X/tonne versus charter-immediately, including correctly flagging elevated risk ahead of the 2023 Red Sea disruption" is falsifiable and judge-verifiable — immune to the "you just made that number up" challenge a single hypothetical shipment invites.
...
9. Is **honest about what's built vs. planned** — every module above is flagged as demo-built, synthetic-but-realistic, or Phase 2 roadmap; nothing is claimed as live that isn't.

---

## 15. Key References / Real-World Grounding
